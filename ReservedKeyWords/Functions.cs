...
namespace AutomationIDELibrary.ReservedKeyWords
{
    public class Functions
    {
        ...
        public Task InjectJQueryAsync(IWebDriver driver)
        {
            InjectJQueryFuncAsync(driver);
            return Task.CompletedTask;
        }

        public Task InjectJQueryScriptAsync(string script, IWebDriver driver)
        {
            InjectJQueryScriptFuncAsync(script, driver);
            return Task.CompletedTask;
        }
        #endregion
        #region BaseFunctions
        public void BaseFunctionClickElementAsync(WebDriverWait driver, By @by)
        {
        ...
        public void BaseFunctionSendKeysAsync(WebDriverWait driver, string input, By @by)
        {
            var element = driver.Until(ExpectedConditions.ElementExists(by)); 
        ...
        public void BaseFunctionSubmitAsync(WebDriverWait driver, By @by)
        {
            var element = driver.Until(ExpectedConditions.ElementExists(by));
            element.Submit();
            _element = element;
        ...
        public void BaseFunctionRedirectAsync(string url, IWebDriver driver)
        {
            driver.Navigate().GoToUrl(url);
        }

        public void BaseFunctionInjectJavaScript(IWebDriver driver, string script, string[] args = null)
        {
            var js = (IJavaScriptExecutor) driver;
            js.ExecuteScript(script, args);
        }

        public void BaseFunctionInjectJavaScriptInnerHTML(IWebDriver driver, string script)
        {
            ...
            newScript.type = ""text/javascript"";"
                + $@"
            newScript.innerHTML = '{scriptSplit[0].TrimStart().TrimEnd()}';
            document.getElementsByTagName('{scriptSplit[1].TrimStart().TrimEnd()}')[0].appendChild(newScript);";

            BaseFunctionInjectJavaScript(driver, innerHtmlScript);
        }

        public void BaseFunctionInjectJavaScriptInnerHTMLPath(IWebDriver driver, string script)
        {
            var scriptSplit = script.Split('|');
            var scriptRead = string.Join("\n", File.ReadAllLines(scriptSplit[0].TrimStart().TrimEnd())).Replace('\n', ' ');
            var innerHtmlScript = @"
            var newScript = document.createElement(""script"");
            newScript.type = ""text/javascript"";"
                                  + $@"
            newScript.innerHTML = '{scriptRead}';
            document.getElementsByTagName('{scriptSplit[1].TrimStart().TrimEnd()}')[0].appendChild(newScript);";

            BaseFunctionInjectJavaScript(driver, innerHtmlScript);
        }

        public void BaseFunctionSleep(string time)
        {
            Thread.Sleep(Convert.ToInt32(time));
        }

        public Task BaseFunctionMessageBoxAsync(string message)
        {
        ...
        public Task BaseFunctionInjectJQuery(IWebDriver driver)
        {
            ...
            BaseFunctionInjectJavaScript(driver, jQueryScript);
            Thread.Sleep(1500);
            return Task.CompletedTask;
        }
        ...
        private Task BaseFunctionOutputTagAsync(ref TextBox tb)
        {
        ...
        private Task InjectJavaScriptFuncAsync(string keyword, IWebDriver driver)
        {
            ...
            else if (keyword.StartsWith("I:JScript", StringComparison.Ordinal))
                BaseFunctionInjectJavaScriptInnerHTML(driver, keyword.Remove(0, 9));
            else if (keyword.StartsWith("IP:JScript", StringComparison.Ordinal))
                BaseFunctionInjectJavaScriptInnerHTMLPath(driver, keyword.Remove(0, 10));
            return Task.CompletedTask;
        }

        private Task InjectJQueryScriptFuncAsync(string keyword, IWebDriver driver)
        {
            if (keyword.StartsWith("JQuery", StringComparison.Ordinal))
                BaseFunctionInjectJavaScriptInnerHTML(driver, keyword.Remove(0, 6));
            else if (keyword.StartsWith("P:JQuery", StringComparison.Ordinal))
                BaseFunctionInjectJavaScriptInnerHTMLPath(driver, keyword.Remove(0, 8));
            return Task.CompletedTask;
        }
        ...
        private Task RunCSScriptFuncAsync(string keyword)
        {
            if (keyword.StartsWith("CSScript", StringComparison.Ordinal))
                ...
                BaseFunctionRunCSScriptAsync(File.ReadAllText(keyword.Remove(0, 10)));
            return Task.CompletedTask;
        }

        private Task InjectJQueryFuncAsync(IWebDriver driver)
        {
            BaseFunctionInjectJQuery(driver);
            return Task.CompletedTask;
        }
        #endregion
    }
